...
library(pastecs) # for stats desc
```

**Load Data**
```{r}
data <- read.csv('study1a_data.csv')
```
...
Sample goes from N = 122 to N= 106. Lose 16 participants.
...
data <- data %>% dplyr::mutate(breadwinning = bread,
                        caregiving = care)
```
...
**Compare expected breadwinning priority between mothers and fathers**
```{r}
# t-test
...
**Compare expected caregiving priority between mothers and fathers**
```{r}
# t-test
